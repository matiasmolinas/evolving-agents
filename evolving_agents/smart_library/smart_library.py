...
class SmartLibrary:
    ...
    async def semantic_search(
        ...
        logger.info(f"[DIAGNOSTIC] semantic_search called with query: '{query}', task_context: '{task_context}'")
        ...
        if query_embedding_orig:
            logger.info(f"[DIAGNOSTIC] query_embedding_orig dimension: {len(query_embedding_orig)}, sample: {query_embedding_orig[:5]}")
        else:
            logger.info("[DIAGNOSTIC] query_embedding_orig is None or empty.")
        # --- End Diagnostic Logging ---
        ...
        if task_context:
            ...
            # --- Start Diagnostic Logging ---
            if task_embedding_for_fallback:
                logger.info(f"[DIAGNOSTIC] task_embedding_for_fallback dimension: {len(task_embedding_for_fallback)}, sample: {task_embedding_for_fallback[:5]}")
            else:
                logger.info("[DIAGNOSTIC] task_embedding_for_fallback is None or empty despite task_context.")
            # --- End Diagnostic Logging ---
        ...
        # --- Start Diagnostic Logging ---
        logger.info(f"[DIAGNOSTIC] MongoDB $vectorSearch Pipeline: {json.dumps(search_pipeline, indent=2)}")
        # --- End Diagnostic Logging ---
